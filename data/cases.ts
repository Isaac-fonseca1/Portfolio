...
export const cases: CaseStudy[] = [
  ...
  // ---------------------------------------------------------------------------
  {
    ...
    problem: {
      pt: "Clínica especializada em terapia ABA operava com um sistema legado — funcional na superfície, mas desatualizado e sem condições de evoluir. Toda informação clínica vivia desconectada, sem auditoria, sem rastreabilidade longitudinal por paciente e sem governança LGPD para dado de criança vulnerável.",
      en: "An ABA therapy clinic was running on a legacy system — functional on the surface, but outdated and with no room to grow. Clinical data lived disconnected — no audit trail, no longitudinal patient tracking, no LGPD-compliant handling of sensitive minor data.",
    },
    solution: {
      pt: "Plataforma própria com agenda multi-terapeuta protegida contra duplo agendamento por lock em banco, prontuário clínico com auditoria total, PEI único e ativo por paciente, autenticação de sessão com TTL configurável e CPF criptografado por HMAC + AES desde o cadastro. Backend Laravel + Postgres com índices únicos parciais e transações com lockForUpdate em todo ponto sensível. Frontend React feature-based com React Query, Zustand e proteção de rota por ability. Arquitetura desenhada para escalar e pronta para integração com sistema financeiro quando o módulo de faturamento entrar em escopo.",
      en: "Custom platform with multi-therapist scheduling protected against double-booking via DB locks, audited clinical records, single-active PEI per patient, session auth with configurable TTL, and CPF encrypted via HMAC + AES at intake. Laravel + Postgres backend with partial unique indexes and lockForUpdate transactions on every sensitive write. Feature-based React frontend with React Query, Zustand, ability-based route guards. Architecture built to scale and ready to integrate with a financial system when billing comes into scope.",
    },
    result: {
